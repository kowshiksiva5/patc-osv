...
- `simulation.html`: controllable connected-sector replay with two stable scenarios, equal vehicle load across PATC and fixed-time modes, movement-group reservations, per-approach signal heads, no-touch vehicle physics, PATC coordination across J/F/S nodes, fixed-signal dwell, queue pressure, severe stops, delay, and completion checks.
